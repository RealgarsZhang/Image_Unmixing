...
int main(){
   ...
   //put the examples into GPU
    ...
    magma_dsetvector(   nn_paramsLength , 
                        nn_params , 1 ,
                      d_nn_params , 1   );
    //It seems that in magma code, "grad" is only for gradient check.
...
//    magma_dmalloc( &(d_delta[0].data) , d_a[0].row*d_a[0].col );
...
//    printf("J2 = %lf\n",J2);
    ...
    //recall that in cost function, the first two arguments are not used.
